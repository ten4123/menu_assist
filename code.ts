...
const MENU_ICON_LIBRARY_NAME = "MenuIconAsset";
...
> = {
    "Manage/invisible": {
        key: "1:9",
        library: MENU_ICON_LIBRARY_NAME
    },
    "Manage/bg": {
        key: "1:7",
        library: MENU_ICON_LIBRARY_NAME
...
function isAssetAvailable(assetName: string): boolean {
    return Boolean(LIBRARY_COMPONENTS[assetName]);
}
...
function appendSpecialRuleAssets(menuName: string, assets: string[]): string[] {
    ...
    if (normalized.includes("별")) {
        const chartAsset = SPECIAL_CHART_ASSETS[assets.length % SPECIAL_CHART_ASSETS.length];
...
function buildKeywordAssetGroups(menuName: string, rows: IconMappingRow[]): AssetGroup[] {
    const groups: AssetGroup[] = [];

    for (const row of rows) {
        const position = getKeywordPosition(menuName, row.keyword);
        const assets = row.assets.filter((assetId) => Boolean(assetId) && isAssetAvailable(assetId));
        if (assets.length === 0) {
            continue;
        }
        groups.push({
...
async function createFallbackFrame(componentName: string): Promise<FrameNode> {
    ...
    fallbackFrame.paddingTop = fallbackFrame.paddingRight = fallbackFrame.paddingBottom = fallbackFrame.paddingLeft = 16;
    fallbackFrame.itemSpacing = 12;
    fallbackFrame.fills = [{ type: "SOLID", color: { r: 0.95, g: 0.95, b: 0.95 } }];
    ...
    fallbackIcon.fills = [{ type: "SOLID", color: { r: 0.85, g: 0.85, b: 0.85 } }];
    ...
    figma.notify(`'${componentName}' 컴포넌트를 찾지 못해 임시 플레이스홀더를 사용합니다.`, { timeout: 4000 });

    return fallbackFrame;
}

async function createIconInstance(componentName: string): Promise<SceneNode> {
    const meta = LIBRARY_COMPONENTS[componentName];

    if (meta?.key) {
        try {
            const libraryComponent = await figma.importComponentByKeyAsync(meta.key);
            const instance = libraryComponent.createInstance();
            instance.name = componentName;
            return instance;
        } catch (error) {
            console.error(`라이브러리 '${meta.library}'에서 '${componentName}' 가져오기 실패`, error);
            figma.notify(`라이브러리 '${meta.library}'에서 '${componentName}' 컴포넌트를 가져오지 못했습니다.`, { timeout: 4000 });
        }
    }
...
function getLocalIconSuggestion(menuName: string): SuggestionResult {
    ...
    const confidence = matchedRows.length > 0 ? Math.min(1, 0.5 + matchedRows.length * 0.2) : 0.2;
...
async function placeIconCombinations(assetCombos: AssetCombination[], menuName: string): Promise<void> {
    ...
    for (const [index, combo] of assetCombos.entries()) {
        try {
            ...
            const sortedCombo = combo.slice().sort((a, b) => getAssetZIndex(a) - getAssetZIndex(b));
