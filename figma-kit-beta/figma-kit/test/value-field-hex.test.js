...
describe('input parsing', () => {
    const TestHexInput = (props) => {
        const { onChange } = props, hexProps = __rest(props, ["onChange"]);
        ...
        <Root>
          <Numeric aria-label={ALPHA_INPUT_LABEL} value={value.a} onChange={setAlpha} min={0} max={1} targetRange={[0, 100]} suffix="%" precision={2}/>
        </Root>
      </Multi>);
    };
    describe('given an input with valid hexedecimal characters', () => {
        ...
        ])('Converts $input to $expected', (_a) => __awaiter(void 0, [_a], void 0, function* ({ input, expected }) {
            render(<TestHexInput />);
            const hexField = screen.getByLabelText(HEX_INPUT_LABEL);
            const alphaField = screen.getByLabelText(ALPHA_INPUT_LABEL);
            yield user.type(hexField, input);
    ...
    describe('given an input with `ignoreAlpha`', () => {
        it('ignores alpha triplet from user input', () => __awaiter(void 0, void 0, void 0, function* () {
            const onChange = vi.fn();
            render(<TestHexInput onChange={onChange} ignoreAlpha/>);
            const hexField = screen.getByLabelText(HEX_INPUT_LABEL);
            yield user.type(hexField, '#FFFFFF00');
            yield user.keyboard('{Enter}');
...
describe('incrementing', () => {
    ...
    function renderAndPrepare(options) {
        return __awaiter(this, void 0, void 0, function* () {
            render(<TestHexInput />);
            const hexField = screen.getByLabelText(HEX_INPUT_LABEL);
            yield user.click(hexField);
            hexField.setSelectionRange(options.selectionStart, options.selectionEnd);
            yield user.keyboard(`{${options.key}}`);
            return hexField;
        });
    }
    it('increments entire hex value when input value is fully selected', () => __awaiter(void 0, void 0, void 0, function* () {
        const hexField = yield renderAndPrepare({ selectionStart: 0, selectionEnd: 6, key: 'ArrowUp' });
        expect(hexField).toHaveValue('818181');
    }));
    it('decrements entire hex value when input value is fully selected', () => __awaiter(void 0, void 0, void 0, function* () {
        const hexField = yield renderAndPrepare({ selectionStart: 0, selectionEnd: 6, key: 'ArrowDown' });
    ...
    describe('given a caret or selection in or around the first hex triplet', () => {
        it.each([
            ...
            { start: 1, end: 2 },
            { start: 0, end: 2 },
        ])('increments red channel', (_a) => __awaiter(void 0, [_a], void 0, function* ({ start, end }) {
            const hexField = yield renderAndPrepare({
        ...
        it.each([
            ...
            { start: 1, end: 2 },
            { start: 0, end: 2 },
        ])('decrements red channel', (_a) => __awaiter(void 0, [_a], void 0, function* ({ start, end }) {
            const hexField = yield renderAndPrepare({
                selectionStart: start,
                selectionEnd: end,
                key: 'ArrowDown',
    ...
    describe('given a selection including any of the characters of red and green triplets', () => {
        it.each([
            ...
            { start: 1, end: 3 },
            { start: 1, end: 4 },
        ])('increments red and green channels', (_a) => __awaiter(void 0, [_a], void 0, function* ({ start, end }) {
            const hexField = yield renderAndPrepare({
                selectionStart: start,
                selectionEnd: end,
                key: 'ArrowUp',
        ...
        it.each([
            ...
            { start: 1, end: 3 },
            { start: 1, end: 4 },
        ])('decrements red and green channels', (_a) => __awaiter(void 0, [_a], void 0, function* ({ start, end }) {
            const hexField = yield renderAndPrepare({
                selectionStart: start,
                selectionEnd: end,
                key: 'ArrowDown',
