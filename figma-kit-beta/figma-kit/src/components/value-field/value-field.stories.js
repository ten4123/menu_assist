...
const MyNumericField = () => {
  ...
  return <ValueField.Numeric value={value} onChange={setValue} />;
    ...
    argTypes: {
        ...
        targetRange: {
            description: "The numeric range that the `value` prop will be normalized to when displayed in the input. Requires non-equal 'max' and 'min' values.",
            table: {
...
const WithLabelComponent = () => {
    const [value, setValue] = useState(0);
    return (<ValueField.Root style={{ width: 100 }}>
...
const Rgba = () => {
    ...
    const handleRChange = (r) => {
        setRgba((rgba) => (Object.assign(Object.assign({}, rgba), { r })));
    };
    const handleGChange = (g) => {
        setRgba((rgba) => (Object.assign(Object.assign({}, rgba), { g })));
    };
    const handleBChange = (b) => {
        setRgba((rgba) => (Object.assign(Object.assign({}, rgba), { b })));
    };
    const handleAChange = (a) => {
        setRgba((rgba) => (Object.assign(Object.assign({}, rgba), { a })));
    };
    return (<div style={{ width: 160 }}>
      <ValueField.Multi>
        <ValueField.Root>
          <ValueField.Numeric value={rgba.r} onChange={handleRChange} min={0} max={1} targetRange={[0, 255]} precision={0}/>
        </ValueField.Root>
        <ValueField.Root>
          <ValueField.Numeric value={rgba.g} onChange={handleGChange} min={0} max={1} targetRange={[0, 255]} precision={0}/>
        </ValueField.Root>
        <ValueField.Root>
          <ValueField.Numeric value={rgba.b} onChange={handleBChange} min={0} max={1} targetRange={[0, 255]} precision={0}/>
        </ValueField.Root>
        <ValueField.Root style={{ flex: '0 0 48px' }}>
          <ValueField.Numeric value={rgba.a} onChange={handleAChange} min={0} max={1} targetRange={[0, 100]} precision={2} suffix="%"/>
        </ValueField.Root>
      </ValueField.Multi>
    </div>);
};
    ...
    return (<div style={{ width: 160 }}>
      <ValueField.Multi>
        ...
        <ValueField.Root style={{ flex: '0 0 62px' }}>
          <ValueField.Numeric value={rgba.a} onChange={setAlpha} min={0} max={1} targetRange={[0, 100]} allowedUnits={['%']} precision={2}/>
