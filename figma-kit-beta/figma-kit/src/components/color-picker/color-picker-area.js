...
const Area = (props) => {
    ...
    const handlePointerEvents = (event) => {
        ...
        handlePositionChange({
            ...
            y: pipe(event.clientY - targetRect.y - CURSOR_VERTICAL_OFFSET, normalize([0, size], [0, 100]), clamp({ min: 0, max: 100 })),
        });
    };
    ...
    return (<div ref={areaRef} className="fp-ColorPickerArea" onPointerDown={handlePointerEvents} onPointerMove={handlePointerEvents} style={{
      ...
      <div tabIndex={0} onKeyDown={handleKeyDown} className={`fp-ColorPickerThumb fp-color-space-${colorSpace}`} style={{
...
function getPosition(colorsByModel, colorModel) {
    if (colorModel === 'hsl') {
